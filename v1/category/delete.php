...
if(empty($_REQUEST['id']) or empty($_REQUEST['reqKey'])){
...
//          show respons          //
$MySQLi->query("DELETE FROM `category` WHERE `id` = '{$id}' LIMIT 1");
@$getData = mysqli_fetch_assoc(mysqli_query($MySQLi, "SELECT * FROM `category` WHERE `title` = '{$title}' LIMIT 1"));
http_response_code(200);
echo json_encode(['status' => 'ok', 'result' => "Done"], JSON_PRETTY_PRINT);
$MySQLi->close();
die();
?>